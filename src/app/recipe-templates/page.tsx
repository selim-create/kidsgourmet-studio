...
import { Home, ChefHat, Filter, Instagram, ImageIcon } from 'lucide-react';
import TemplateCard from '@/components/TemplateCard';
import { RECIPE_TEMPLATES } from '@/lib/templates';
...
export default function RecipeTemplatesPage() {
  ...
  const filteredTemplates = RECIPE_TEMPLATES.filter(template => {
  ...
  return (
    <main className="min-h-screen bg-[#121212] text-white font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              <ChefHat size={24} className="text-[#FF7F3F]" />
              Tarif Şablonları
            </h1>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        ...
        {/* Page Header */}
        <div className="mb-12">
          <h2 className="text-4xl font-bold mb-4">Tarif Şablonları</h2>
          <p className="text-lg text-gray-400">
            Bebek ve çocuk tarifleri için özel olarak tasarlanmış {RECIPE_TEMPLATES.length} farklı şablon
          </p>
        </div>
        ...
        <div className="mb-8 flex items-center gap-4 bg-[#1E1E1E] border border-white/10 rounded-2xl p-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <Instagram size={16} />
              Story ({RECIPE_TEMPLATES.filter(t => t.format === 'story').length})
            </button>
            ...
            >
              <ImageIcon size={16} />
              Post ({RECIPE_TEMPLATES.filter(t => t.format === 'post').length})
            </button>
          </div>
        </div>
